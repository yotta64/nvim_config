...
local function pick_telescope(title, items, on_select)
  ...
  if not ok then
    vim.ui.select(items, { 
      prompt = title, 
      format_item = function(item) return item.name or item end 
    }, on_select)
    return
  end
...
local function execute_commit(buf)
  ...
  if exit_code == 0 then
      ...
      if vim.v.shell_error == 0 then
          ...
          vim.notify("❌ Errore (File non tracciati?):\n" .. output, vim.log.levels.ERROR)
      end
  else
...
function M.open_ghost_commit()
  ...
  if ok and gn and gn.get_commit_context_summary then
      ...
      if #context_lines > 0 then
          ...
          for _, l in ipairs(context_lines) do table.insert(summary_lines, l) end
      end
  end
  ...
  if #summary_lines > 0 then
      ...
      for _, l in ipairs(summary_lines) do table.insert(content, l) end
  end
  ...
  local win = api.nvim_open_win(buf, true, {
      ...
      style = "minimal", border = "rounded", title = " 👻 Ghost Commit ", title_pos = "center"
  })
  ...
  vim.keymap.set("i", "<C-s>", function() execute_commit(buf) end, opts)
  vim.keymap.set("n", "<C-s>", function() execute_commit(buf) end, opts)
  vim.keymap.set("n", "ZZ", function() execute_commit(buf) end, opts)
end
...
local function on_git_head_change()
  vim.schedule(function()
    vim.cmd("checktime")
    if package.loaded["neo-tree"] then vim.cmd("Neotree git_status") end
    local branch = fn.system("git rev-parse --abbrev-ref HEAD"):gsub("\n", "")
    vim.notify("Git Branch changed: " .. branch, vim.log.levels.INFO, { title = "GhostVault" })
    api.nvim_exec_autocmds("User", { pattern = "GhostGitChange" })
  end)
end

local function start_git_watcher(root_path)
  if M.git_watcher then uv.fs_event_stop(M.git_watcher); M.git_watcher = nil end
  local git_head = root_path .. "/.git/HEAD"
  if fn.filereadable(git_head) == 0 then return end 

  M.git_watcher = uv.new_fs_event()
  uv.fs_event_start(M.git_watcher, git_head, {}, function(err, filename, events)
      if not err then on_git_head_change() end
  end)
end

function M.git_actions()
    if not M.project_root then vim.notify("Nessun progetto attivo", vim.log.levels.WARN); return end

    local actions = {
        { name = "✨ Ghost Stage (Add/Reset)", cmd = "stage" },
        { name = "✏️  Ghost Commit (Smart Draft)", cmd = "commit" },
        { name = "🚀 LazyGit", cmd = "lazygit" },
        { name = "🌿 Switch Branch", cmd = "switch" },
        { name = "📂 Git Status", cmd = "status" },
        { name = "📥 Pull", cmd = "pull" },
        { name = "📤 Push", cmd = "push" },
    }

    pick_telescope("Ghost Git Actions", actions, function(choice)
        if choice.cmd == "stage" then
          M.git_stage_interactive()

        elseif choice.cmd == "commit" then
           M.open_ghost_commit()
        
        elseif choice.cmd == "lazygit" then
            -- Supporto per plugin dedicato (kdheepak/lazygit.nvim)
            if vim.fn.exists(":LazyGit") == 2 then
                vim.cmd("LazyGit")
            -- Fallback su ToggleTerm
            elseif fn.executable("lazygit") == 1 then
                vim.cmd("TermExec cmd='lazygit' direction=float name='GhostGit'")
            else
                vim.notify("LazyGit non trovato.", vim.log.levels.WARN)
            end
        
        elseif choice.cmd == "switch" then
             local branches = fn.systemlist("git branch --format='%(refname:short)'")
             pick_telescope("Checkout Branch", branches, function(branch)
                if branch then
                    -- Esegui checkout e cattura output/errori
                    local out = fn.system("git checkout " .. branch)
                    if vim.v.shell_error ~= 0 then
                        vim.notify("⚠️ Impossibile cambiare branch:\n" .. out, vim.log.levels.ERROR)
                    else
        ...
        elseif choice.cmd == "status" then
             ...
             if ok then builtin.git_status() else vim.cmd("!git status") end

        elseif choice.cmd == "pull" then
            vim.notify("Pulling...", vim.log.levels.INFO)
            vim.fn.jobstart("git pull", {
                on_exit = function(_, code)
                    if code == 0 then vim.notify("Pull Completato!", vim.log.levels.INFO)
                    else vim.notify("Errore Pull.", vim.log.levels.ERROR) end
                end
            })
        elseif choice.cmd == "push" then
             vim.fn.jobstart("git push", {
                on_exit = function(_, code)
                    if code == 0 then vim.notify("Push Completato!", vim.log.levels.INFO)
                    else vim.notify("Errore Push.", vim.log.levels.ERROR) end
                end
            })
        end
    end)
...
function M.restore_session()
  ...
  if fn.filereadable(session_path) == 1 then
    ...
    if not ok then vim.notify("Session load warning: " .. err, vim.log.levels.WARN) end
    ...
    vim.ui.select(
      ...
      function(choice)
        ...
        if choice:find("Personale") then 
          -- Personale: Ignora tutta la cartella .ghost
          f_app:write(VAULT_DIR .. "/\n")
        else 
          -- Collaborativo: Ignora solo sessione e config locale
          f_app:write(VAULT_DIR .. "/" .. STATE_FILE .. "\n")
          f_app:write(VAULT_DIR .. "/" .. LOCAL_CONFIG .. "\n")
          -- Nota: notes/ e identity.json vengono tracciati
        end
...
-- ============================================================
-- 7. Ghost Stage (Interactive Git Add) - ICONE CHIARE
-- ============================================================

local function get_git_status_items()
  local lines = fn.systemlist("git status --porcelain")
  local items = {}
  ...
  for _, line in ipairs(lines) do
    if line ~= "" then
        local x = line:sub(1, 1) -- Index (Staged)
        local y = line:sub(2, 2) -- WorkTree (Unstaged)
        local file = vim.trim(line:sub(4))
        
        -- Default
        local status = "untracked"
        -- Icone Nerd Font classiche per Git
        local icon = " "     -- Untracked (Question mark)
        local hl = "Comment"  -- Grigio/Rosso spento
        ...
        table.insert(items, {
            ...
            icon = icon,
            hl = hl,
            display = icon .. " " .. file -- Semplice stringa per la visualizzazione
        })
    end
  end
  return items
...
function M.git_stage_interactive()
  local ok, pickers = pcall(require, "telescope.pickers")
  ...
  -- Configurazione Display: Icona colorata + Nome file
  local displayer = entry_display.create {
    separator = " ",
    items = {
      { width = 2 }, -- Icona
      { remaining = true }, -- Nome file
    },
  }

  local make_display = function(entry)
    return displayer {
      { entry.icon, entry.hl },      -- Applica colore solo all'icona
      { entry.value, "TelescopeNormal" }, -- Testo normale
    }
  end

  local function refresh_picker(prompt_bufnr)
      local current_picker = action_state.get_current_picker(prompt_bufnr)
      local new_items = get_git_status_items()
  ...
  pickers.new({}, {
    prompt_title = "👻 Ghost Stage (<CR>: Toggle | <C-a>: All | <C-r>: Reset)",
    finder = finders.new_table({
      results = get_git_status_items(),
      entry_maker = function(entry)
        return {
          value = entry.value,
          display = make_display,
    ...
    sorter = conf.generic_sorter({}),
    previewer = conf.file_previewer({}), -- Aggiunto preview del file (utile per vedere diff)
    attach_mappings = function(prompt_bufnr, map)
      ...
      actions.select_default:replace(function()
        ...
        local file = selection.path
        local status = selection.status
        
        if status == "staged" then
            fn.system("git reset -- " .. file)
        else
      ...
      map("i", "<C-a>", function()
          fn.system("git add -A")
          vim.notify("Tutti i file aggiunti.", vim.log.levels.INFO)
          refresh_picker(prompt_bufnr)
      end)

      map("i", "<C-r>", function()
          fn.system("git reset")
          vim.notify("Stage svuotato.", vim.log.levels.INFO)
          refresh_picker(prompt_bufnr)
      end)
